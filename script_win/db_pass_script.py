...
logging.basicConfig(filename='db_pass_log.log',level=logging.DEBUG)
...
for lite_row in lite_rows:
	...
	def add_to_cmd(cmd,i,arg, val):
		if type(val)==str:
			if re.match("[^A-Za-z0-9]",val):
				pass#print "not parsing a null string argument"+arg
			else:
				cmd=cmd+"--"+arg+"="+val+"  "
			i=i+1
			#print i,':',cmd
	...
	if stdout:
			lite_cur.execute("update inv_data_bk set  status='Y' where prod_cd ='" +lite_row[0]+"';")
			lite_con.commit()
			print stdout.read()
