...
if confirm_char=='Y':
	...
	def delete_database(): #function for deleting the temporary DB
		connection, cursor= connection_script.connect_to_sqlite() #connecting to DB
		cursor.execute("""DROP TABLE inv_data_bk;""") #deleting table
		cursor.close()
		connection.close()
		
	def create_database(): #function for creating the temporary DB
		connection, cursor= connection_script.connect_to_sqlite()#connecting to DB
		cursor.execute(create_query)#deleting table
		connection.commit()
		cursor.close()
		connection.close()
