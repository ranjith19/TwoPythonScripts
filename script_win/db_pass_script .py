...
lite_cur.execute("select prod_cd from inv_data_bk where status='U' or status='N';")
for lite_row in lite_cur:
	ms_cur.execute("select prod_cd,whs_num,in_stock,lastrcv_qty,lastrcv_dt,price_base,frt_cus,prod_duty,handl_fee,misc_fee,avg_cost,lt_sl_dt,vendor,lst_order,ord_dt,stk_ind,back_qty,order_qty,on_order_qty,wip_qty,rma_qty,water_qty,ordersize,minstock,inv_loc,unit_color,class_cd,descrip,def_unit,updt_dt,phyc_dt,image_nm,oem_cd,alt_cd,updt_by,currency_cost,cost_factor  from inv_data_bk where prod_cd='"+lite_row[0]+"';")
	row_status=ms_row.status
	if status=='N':
	...
	if stdout:
			lite_cur.execute("update inv_data_bk set  status='Y' where prod_cd ='" +prod_cd+"';")
			lite_con.commit()
			print stdout.read()
	print 'yes ', prod_cd
	if stderr:
